...
public class MenuRequestController {
    ...
    // GET /api/v1/menus/search?keyword=coffee&categoryCode=2&page=1
    @GetMapping("/search")
    public ResponseEntity<Map<String, Object>> searchMenus(
            // @RequestParam Map: 요청 파라미터 전체를 Map으로 받음
    ...
    public ResponseEntity<Map<String, Object>> findMenu(
            // @PathVariable: URL 경로 일부를 메서드 파라미터로 받음
    ...
    @PostMapping
    public ResponseEntity<Map<String, Object>> creatMenu(
            // 요청 본문의 JSON을 읽어서 MemuDto 객체로 변환하여 파라미터로 넣어줘!
    ...
    public ResponseEntity<Map<String, Object>> requestInfo(
        ...
        response.put("user-agent", userAgent);
    ...
    public ResponseEntity<Map<String, Object>> saveSession(
            HttpSession session,    // 현재 요청의 session 객체를 넣어줌
