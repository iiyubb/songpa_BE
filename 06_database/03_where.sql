...
SELECT
    ...
    orderable_status
FROM tbl_menu
WHERE orderable_status <> 'Y'; -- 이 조건이 true인 행(row)들만 결과에 포함
...
SELECT
    ...
    orderable_status
FROM tbl_menu
WHERE orderable_status = 'Y' OR cagegory_code = 10;
...
-- LIKE: 문자열 패턴 검색
-- 메뉴 이름에 '마늘' 포함된 메뉴 찾고 싶을 떄, %는 0개 이상의 모든 문자를 의미하는 와일드 카드
SELECT
    menu_name
FROM tbl_menu
WHERE menu_name LIKE '%마늘%'; -- 부정은 NOT LIKE
...
SELECT
    ...
    ref_category_code
FROM tbl_category
-- ref_category_code = NULL; -- NULL 값 비교에는 = 사용할 수 없음
WHERE ref_category_code IS NULL; -- 부정은 IS NOT NULL
